...
-- Hardcoded affix data: [affixID] = {name, filedataid, description}
-- IDs соответствуют кастомному серверу uWoW (отличаются от retail!)
-- Сравнение с retail: Бесплотный retail=148 -> uwow=136; Усиливающий retail=152 -> uwow=7
-- Иконки (filedataid) взяты с wowhead.com/ru/affixes
local mythic_plus_affix_data = {
  -- id  | Название         | Иконка (FileDataID) | Описание
  [136] = {"Бесплотный",    298642,  "Некоторые не-боссы периодически становятся бесплотными и не получают урона. Используйте способности контроля или отвлечения, чтобы прерывать этот эффект."},
  [11]  = {"Взрывной",      1035055, "Некоторые не-боссы взрываются при смерти, нанося смертельный урон ближайшим игрокам. Немедленно отходите от умирающих врагов."},
  [13]  = {"Взрывоопасный", 2175503, "Некоторые враги призывают Взрывные сферы. Если сферу не уничтожить, она нанесёт огромный урон всей группе."},
  [3]   = {"Вулканический", 451169,  "Под ногами случайных игроков возникают гейзеры, наносящие урон от огня и подбрасывающие в воздух. Следите за маркерами на земле и отходите."},
  [135] = {"Изнемогающий",  237555,  "В подземелье появляются страдающие NPC. Исцельте или обездвижьте их до окончания таймера; при смерти они дают противникам мощное усиление."},
  [123] = {"Злопамятный",   135945,  "При гибели не-боссов появляются Злопамятные духи, атакующие случайных игроков. Используйте способности контроля или убивайте духов быстро."},
  [8]   = {"Кровавый",      136124,  "Когда не-боссы умирают, они оставляют на земле лужи крови, восстанавливающие здоровье противникам и наносящие урон игрокам. Оттягивайте врагов от луж."},
  [4]   = {"Некротический", 1029009, "Все атаки врагов накладывают на цель эффект Некротической раны, снижающий максимальное здоровье. При 0% – мгновенная смерть. Очищается вне боя."},
  [134] = {"Оплетающий",    134412,  "Периодически определённые области покрываются корнями, замедляя и обездвиживая игроков. Заблаговременно выходите из зоны появления корней."},
  [6]   = {"Разъяренный",   132345,  "Некоторые не-боссы приходят в ярость при потере 30% здоровья, получая значительный прирост к урону и скорости атаки. Используйте контроль и кулдауны."},
  [14]  = {"Сотрясающий",   136025,  "Периодически игроки получают эффект Сотрясения: движение прерывает заклинание и оглушает. Заканчивайте каст до применения эффекта."},
  [9]   = {"Тиранический",  236401,  "Боссы подземелий получают 30% к здоровью и 15% к урону. Готовьте кулдауны и экономьте ресурсы для боссов."},
  [10]  = {"Укрепленный",   463829,  "Не-боссы получают 20% к здоровью и 30% к урону. Будьте осторожны с большими пакетами врагов и старайтесь контролировать количество."},
  [7]   = {"Усиливающий",   132333,  "Некоторые не-боссы усиливают соседних союзников, наделяя их дополнительными способностями или увеличивая наносимый урон. Убивайте усиливающих врагов в первую очередь."},
  [124] = {"Бушующий",      136018,  "Некоторые не-боссы создают смерчи, которые подбрасывают и дезориентируют игроков. Держитесь подальше от вихрей, особенно в бою."},
}

WeakAuras.mythic_plus_affixes = {}
WeakAuras.mythic_plus_affix_descs = {}

for id, data in pairs(mythic_plus_affix_data) do
  local affixName, affixDesc, affixIcon = C_ChallengeMode.GetAffixInfo(id)
  local displayName = affixName or data[1]
  local displayIcon = (affixIcon and affixIcon > 0) and affixIcon or data[2]
  local displayDesc = (type(affixDesc) == "string" and #affixDesc > 0) and affixDesc or data[3]
  WeakAuras.mythic_plus_affixes[id] = "|T" .. displayIcon .. ":16:16:0:0|t " .. displayName
  WeakAuras.mythic_plus_affix_descs[id] = displayDesc
end
